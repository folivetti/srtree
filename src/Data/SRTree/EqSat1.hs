...
recalculateBest costFun eid =
    ...
    where
        ...
        nodeCost costMap enode =
          do optChildren <- traverse (costMap Map.!?) (children enode)
        ...
        fillUpCosts classes m = let mm = IntMap.foldrWithKey costOfClass (False, m) classes
                                in case mm of
                                    (False, _) -> m
                                    (True, m') -> fillUpCosts classes m'
        costOfClass :: EClassId -> EClass -> (Bool, CostMap) -> (Bool, CostMap)
        costOfClass eid ecl (b, m) = let currentCost = m Map.!? eid
                                         minCost     = minimumBy' (compare `on` fst) $ mapMaybe (nodeCost m) $ Set.toList $ _eNodes ecl
                                      in case (currentCost, minCost) of
                                           (_, Nothing)         -> (b, m)
...
matchWithScheduler db it ruleNumber rule =
  do mbBan <- gets (IntMap.!? ruleNumber)
     if False -- mbBan == Nothing || (fromJust mbBan) > it
        then pure []
