...
eGraphToDatabase egr = foldrWithKeyNM' addENodeToDB (DB mempty) (egr^._memo)
  where
    ...
    addENodeToDB enode classid (DB m) =
        ...
        if not_too_high_cl classid egr
          then DB $ M.alter (Just . populate (classid:children enode)) (operator enode) m
          else DB m
...
not_too_high_cl :: Language l => ClassId -> EGraph a l -> Bool
not_too_high_cl cId egr = getHeight 8 cId
  where
    getParents c = filter (\ci -> canon ci /= canon c) $ toList $ fmap fst $ egr ^._class c . _parents
    getSame    c = filter (\ci -> canon ci == canon c) $ toList $ fmap fst $ egr ^._class c . _parents
    canon c = find c egr
    getHeight 0 c = False
